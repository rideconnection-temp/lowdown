/* @projectDescription jQuery Password Strength Plugin - A jQuery plugin to provide password strength functions
...
digitalspaghetti.password = {	
	'defaults' : {
		'displayMinChar': true,
		'minChar': 8,
		'minCharText': 'You must enter a minimum of %d characters, including at least one each of uppercase and lowercase letters, numbers, and symbols',
		'colors': ["#f00", "#c06", "#f60", "#3c0", "#3f0"],
		'scores': [20, 30, 43, 50, 99999999],
	...
	'rules' : {
		...
		'upper_lower_combo': false,
		'letter_number_combo': false,
	...
	'calculateScore': function (word) {
		...
		var lengths = [1,25,50,75,99];
		for (var i = 0; i < 5; ++i) {
		    if (digitalspaghetti.password.totalscore <= digitalspaghetti.password.options.scores[i]) {
			digitalspaghetti.password.strColor = digitalspaghetti.password.options.colors[i];
			digitalspaghetti.password.strText = digitalspaghetti.password.options.verdicts[i];
			digitalspaghetti.password.width =  lengths[i];
			break;
		    }
		}
		jQuery('.password-strength-bar').stop();
			
		if (digitalspaghetti.password.options.displayMinChar && !digitalspaghetti.password.tooShort && digitalspaghetti.password.totalscore >= 25 ) {
